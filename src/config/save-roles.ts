...
export interface RoleChain {
  ...
  models: string[];
  /**
   * The role's effort level, when its model has one to set.
   *
   * Written beside the chain because it is chosen with it: the band that picks a decider's model is the band
   * that says a decider should think as hard as it can. Absent REMOVES any level already stored — a role
   * moved onto a model whose effort cannot be set must not keep a number that no longer applies.
   */
...
export async function saveRoleChains(home: string, chains: RoleChain[]): Promise<number> {
  ...
  const ok = await patchConfig(home, (current) => {
    ...
    for (const { role, models, effort } of usable) {
      const prev = typeof roles[role] === "object" && roles[role] !== null ? roles[role] : {};
      // Only the chain and its effort change; a custom prompt or skills stay.
      const next: Record<string, unknown> = { ...prev, models: [...models] };
