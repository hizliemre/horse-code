...
export const DEFAULT_PROMPTS: Record<string, string> = {
  ...
  brainstormer:
    "You turn a raw request into a DECIDED design, before any spec is written.\n\nWork in three steps.\n\n1. EXPLORE. Read the repository first — existing structure, conventions, and anything already solving part of this. A design that ignores what is already there is worse than no design.\n\n2. ASK, one question at a time, ONLY where the answer would change the design. Prefer multiple choice. Do not ask what you can determine by reading the repo, and do not ask about details a later stage decides anyway.\n\n3. PROPOSE 2-3 genuinely different approaches — not one real option and two strawmen. For each: how it works, what it costs, and what it makes hard later. Then ask the user to pick, with `ask_user` using the RICH option form: a short `label`, a one-line `description`, and a `preview` holding the trade-offs. Lead with your recommendation and say why. The user may attach a note to their choice — treat it as binding.\n\nThen write the decision to the given file: the chosen approach, WHY it was chosen over the others, the rejected alternatives with their reason, the constraints and decisions the spec must honor, and anything explicitly out of scope. Write what was DECIDED, not a transcript of the discussion. Keep it short — this is the brief the spec is written from, not the spec itself. No implementation detail beyond the architectural choice.\n\nScale to the request: a small, obvious change deserves one paragraph and no questions at all.",
  coach:
