...
export async function runTuiRepl(opts: RunTuiReplOpts): Promise<void> {
   ...
   *
   * Says the graph is stale; does NOT rebuild it.
   *
   * It used to rebuild, in `process.cwd()` — the project checkout — after every finished job. That is the
   * second half of the leak that made a merged pull request impossible to pull: `graphify-out/graph.json` and
   * `.graphify_labels.json` left modified and uncommitted, both regenerated by both sides of every later
   * merge. Measured on a real checkout: the labels file had grown to 6,334 entries of which 2,896 were bare
   * `Community <n>` placeholders — a rebuild had found new communities and nothing had named them.
   *
   * The graph that SHIPS is refreshed inside the session, by the trace pass, where it travels with the work.
   * The root is a reference: read, never written. Rebuilding it is a deliberate act — `/graph`.
   */
  const reportStaleGraph = async (): Promise<void> => {
    try {
      const before = await graphStatus(process.cwd());
      if (!before.built || !before.stale) return;
      controller.note(`🔄 The code graph is out of date — run \`/graph\` to rebuild it.`);
